...
public class SRService {
    ...
    public List<String> createSR(Context context, Oxid oxid, SRHolder srHolder) throws CommonWebException, MatrixException {
        ...
        if (var4 != null) {
            log.info("[createSR] Create SR for {} {} {} with {} paths", oxid.getKind(), oxid.getType(), oxid.getPhysicalid(), srHolder.getPaths().size());
        } else {
            log.warn("[createSR] No Path provided for SR creation for {} {} {} ", oxid.getKind(), oxid.getType(), oxid.getPhysicalid());
        }

        if (!this.isAuthorizedOwner(context, oxid)) {
            log.error("[createSR] Owner type not supported {}. For this type, no role/semantics available in metadata files {} ", oxid.getType(), "VPMCfgEffectivity|RFLPLMFunctional");
            ...
            while(true) {
                ...
                do {
                    if (!var5.hasNext()) {
                        log.info("<<[createSR] : srCreatedIds {}", var6.toString());
                        return var6;
                    }
                ...
                log.debug("[createSR] Path({}): [IDRel]={},[Semantics]={},[Role]={},[RoleSemantics]={},[PrivateData]={},[AppIndex]={},[OutOfScope]={},[lastPIDAndRoleValue]={}", new Object[]{var8, var13, semanticsByNames.get(srHolder.getSemantics()), rolesByNames.get(srHolder.getRole()), srHolder.getRole() + srHolder.getSemantics(), srHolder.getPrivateData(), var7 - 1, var14, var15});
                ...
                if (var17 != null) {
                    ...
                    log.debug("[createSR] SR Quad is not empty : {}", var18);
                ...
                try {
                    ...
                    log.error("[createSR] Exception occured on addPath call for {} {} {} ", oxid.getKind(), oxid.getType(), oxid.getPhysicalid());
                    throw var25;
                }
            }
        }
    ...
    private Path.Element[] getPathElements(Context var1, PathHolder var2, String var3) throws CommonWebException, MatrixException {
        ...
        if (var2.getPathMatrixElements() != null) {
            ...
            if (!var8.isEmpty()) {
                ...
                log.debug("[getPathElements]  query bus {} on {} returned : {} ", var6, var28, var29);
                ...
                for(var31 = var29.iterator(); var31.hasNext(); var7[var22] = new Path.Element(var22, var27, var24, var19, var21, var20, var18, var25)) {
                    ...
                    log.debug("[getPathElements] Element({}): [Kind]={},[Type]={},[PhysicalId]={},[MajorId]={},[LogicalId]={},[updatestamp]={},[Relevancy]={}", new Object[]{var22, Integer.valueOf(var27), var24, var19, var21, var20, var18, var25});
                }
            }
            ...
            if (!var9.isEmpty()) {
                ...
                log.debug("[getPathElements]  query rel {} on {} returned : {} ", var6, var28, var30);
    ...
    private int getNextIdRelValue(Context var1, Oxid var2, int var3) throws MatrixException {
        log.info(">>[getNextIdRelValue]");
        String var4 = var2.getPhysicalid();
        log.info("Get last IDRel from SR Owner {}, initial value: {} ", var4, var3);
        int var5 = var3;
        ...
        if (var6 != null) {
            ...
            if (var2.getKind().equals(BaseKind.businessobject)) {
                ...
                log.debug("[getNextIdRelValue]  query bus {} on {} returned : {} ", var7, var2.getPhysicalid(), var14);
                ...
                while(var11.hasNext()) {
                    ...
                    log.debug("[getNextIdRelValue]  IDRel : {}", var13);
                    if (var13 != null) {
                ...
                log.debug("[getNextIdRelValue]  query rel {} on {} returned : {}", var7, var2.getPhysicalid(), var10);
            ...
            log.debug("[getNextIdRelValue]  List of retrieved IDRels : {}", var8);
            if (!var8.isEmpty()) {
                ...
                log.debug("[getNextIdRelValue]  Maximum IDRel value from listOfIDrels : {} ", var5);
                ++var5;
            }
        ...
        log.info("<<[getNextIdRelValue] iIDrel : {} ", var5);
        return var5;
    }
    ...
    private String getOutOfScopeForElements(List<com.dassault_systemes.platform.model.PathHolder.Element> var1) throws CommonWebException {
        log.info(">>[getOutOfScopeForElements]");
        ...
        for(int var3 = 0; var3 < var1.size(); ++var3) {
            ...
            if (null == var4) {
                log.error("[getOutOfScopeForElements] Current PathElement is null");
