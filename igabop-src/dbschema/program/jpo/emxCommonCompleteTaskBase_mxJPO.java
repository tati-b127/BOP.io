  ...
  public static final String POLICY_VPLM_SMB_DEFINITION = "VPLM_SMB_Definition";
    ...
    //Start: Auto-Stop
            ...
            if (isRouteToBeStopped) {
                if ("Immediate".equals(sAutoStopOnRejection) || isLevelCompleted) {
					...
					sendNotification(context, sRouteActionOfTask, ObjectId, sRouteId,  sApprovalStatus,  sRouteName,  sLastName,  sFirstName,  sPerson, sType, sName, sRev, sRouteType, sRouteRev,  sActualCompletionDate,  sComments );
    ...
    */
                            ...
                            objectIds [0] = sTaskId;
                                ...
                                inboxTaskToComplete.setAttributeValue(context, sAttComments, EnoviaResourceBundle.getProperty(context,"emxFrameworkStringResource", strLocale, "emxFramework.ProgramObject.eServicecommonCompleteTask.AutoCompleteCommment")+" "+sType+" "+sName+" "+sRev);
                            ...
                            arguments[0]  = sTaskOwner;
                                arguments[1]  = "emxFramework.ProgramObject.eServicecommonCompleteTask.SubjectTaskAutoComplete";
                            arguments[2]  = "0";
                                arguments[3]  = "emxFramework.ProgramObject.eServicecommonCompleteTask.MessageTaskAutoComplete";
                            arguments[4]  = "6";
                            arguments[5]  = "IBType";
                ...
                if (bFound == 0 && sRouteStatus.equals("Started")) {
                    ...
                    if (outStr1 == 0) {
                        //IGABOP start. Here is the moment when Route is completed and has at least one Route content
                        String bopExt = MqlUtil.mqlCommand(context, "print bus $1 select interface $2 dump", sRouteId, "IGABOPRoute");
                        if(Arrays.asList(bopExt.split(",")).contains("IGABOPRoute"))
                            IGABOPReport_mxJPO.generateReport(context, sRouteId);
                        //IGABOP end
                        ...
                        if (sRouteCompletionAction.equals("Promote Connected Object")) {
                           ...
                           MapList objectlist= Route.getRelatedObjects(context,
                            ...
                            for(int count=0; count<objectlist.size();count++) {
                                ...
                                if(POLICY_VPLM_SMB_DEFINITION.equals(sObjPolicy) && !STATE_RELEASED.equals(sObjCurrent)) {
                                    String sSign = PROMOTE_STATE_SIGN.get(sObjCurrent);
                                    MqlUtil.mqlCommand(context, "unsign bus $1 signature '$2' comment '$3'", sObjId, "all", "Ok");
                                    MqlUtil.mqlCommand(context, "approve bus $1 signature '$2' comment '$3'", sObjId, sSign, "Ok");
                                    sIsObjSatisfied = "TRUE";
                                } else if (sObjBaseState.equals("Ad Hoc") && (!sObjBaseState.equals(sObjCurrent) || !sObjBasePolicy.equals(sObjPolicy))) {
                                    continue;
                                }
                                ...
                                for(int i=0; i<ObjectList.size() ; i++) {
                                    ...
                                    if (("Ad Hoc".equals(sObjRouteBaseState)) || (sObjRouteBaseState.equals(sObjCurrent) && sObjRouteBasePolicy.equals(sObjPolicy))) {
    ...
    private static void sendNotification(Context context, String sRouteActionOfTask, String ObjectId, String sRouteId, String sApprovalStatus, String sRouteName, String sLastName, String sFirstName, String sPerson,String sType,String sName,String sRev,String sRouteType,String sRouteRev, String sActualCompletionDate, String sComments ) throws Exception{
           ...
           if ("Approve".equals(sRouteActionOfTask)) {
               if ("Reject".equals(sApprovalStatus)) {
                   ...
                   String[] tableValues = {strTranslatedRouteActionOfTask, sRouteLink, sInboxTaskLink, sPerson, sActualCompletionDate, sComments};
